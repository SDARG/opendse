...
public class ActivationEncoderDefault implements ActivationEncoder {
	...
	public Set<Constraint> toConstraints(CommunicationFlow communicationFlow, Architecture<Resource, Link> routing) {
		...
		for (DirectedLink directedLink : Models.getLinks(routing)) {
			activationConstraints.add(Constraints.generateNegativeImplication(communicationFlow.getSourceDTT(), Variables.varDDLRR(communicationFlow, directedLink)));
			activationConstraints.add(Constraints.generateNegativeImplication(communicationFlow.getDestinationDTT(), Variables.varDDLRR(communicationFlow, directedLink)));
		}
